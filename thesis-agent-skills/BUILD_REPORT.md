...
- Constitution, read-only overlay example, Hermes controller prompt/configuration, disabled
  cron prompt, controller/worker/reviewer installers, reproducible packaging and checksums.
...
├── hermes/{controller-profile,examples}/
├── installers/{install.ps1,install.sh,install.py,install_workflows.py}
├── tests/{test_contracts.py,test_forward_compute.py,
│          test_forward_evidence.py,test_forward_workflow.py,validation-results.json}
...
arrays and preserve run IDs. No Telegram client or second scheduler/engine was created. The
current upstream shell step uses shell execution, so no shipped workflow uses it. Full lifecycle
...
Watch-once is read-only and emits nothing for the same meaningful-state fingerprint. Hermes
owns delivery and acknowledgement; only a successful delivery advances the cursor. Human
approval is never inferred or generated by runtime helpers. Execute and install default to
preview. Remote/local target changes require an updated manifest and approval.
...
| Hermes reference | 8f1542422b6806a20f8248d582db2f47b5cb34cc; docs accessed 2026-09-19 |
...
3. Hermes profile loading, cron delivery, Telegram pairing and live Codex/Perplexity operation
   were reviewed against documentation, not connected or run. No job or gateway was enabled.
...
5. Approval records are authenticated operationally by Hermes access controls and filesystem
   ownership, not cryptographic signatures. A malicious agent with write access to approvals or
   code can forge local records. Use protected approval storage and OS isolation as appropriate.
...
provide missing thesis inputs, ratify scope/constitution, configure the existing Hermes gateway,
