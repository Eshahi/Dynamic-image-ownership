...
description: "Operate installed thesis Spec Kit runs and relay exact human gate decisions through an optional deterministic Telegram bridge."
---
...
For notification polling, keep the last successfully delivered token under the ignored `.thesis-notify/` project directory. Pass it as `--previous-token`; unchanged state emits no stdout. Update the cursor only after successful delivery. Bound workflow messages to 1,500 characters. Polling alone must never resume a run.
...
The optional `telegram_bridge.py` transport uses the Bot API directly and no language model. Read `telegram/README.md` before live configuration. It requires an exact private user ID and chat ID allowlist, accepts only `/status`, `/approve` and `/reject`, and never executes free-form text. A Telegram approval or rejection must name the exact current run and step. The bridge creates the short-lived decision record only after authenticating that exact command; Spec Kit remains the sole workflow state owner. Do not confuse workflow approval with separate compute-spending approval.

Authorization: local read-only status needs no new permission. Start/resume require the user's workflow instruction; gate decisions require explicit authenticated human evidence. Never infer approval from casual text, notification delivery, or polling.

## Input/output contract

Inputs must match the strict JSON schemas in `scripts/_runtime/thesis_agents/schemas` after installation, or the bundle's `src/thesis_agents/schemas` during development. Stable JSON status, bounded notification, approval review package, normalized guide JSON and optional Telegram delivery state.
