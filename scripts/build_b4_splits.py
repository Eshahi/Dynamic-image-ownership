...
def run(manifest_path,dev_path,metadata_path,canonical_path,canonical_summary,config_path,output_root):
    ...
    manifest_raw=bound(manifest_path,MANIFEST_SHA,8*1024*1024)
    dev_raw=bound(dev_path,DEV_SHA,64*1024)
    ...
    if config["manifest_sha256"]!=MANIFEST_SHA or config["development_ids_sha256"]!=DEV_SHA:
    ...
    for row in manifest:
        ...
        selected.append(coverage_join(row,observed))
    ...
    inputs={"manifest_sha256":MANIFEST_SHA,"dev_ids_sha256":DEV_SHA,"private_metadata_bridge_sha256":META_BRIDGE_SHA,
    ...
    json_output(output_root/"sample-size-check.json",{
        "status":"BLOCKED_DECISION_provisional_engineering_allocation_not_scientific_freeze","inputs":inputs,
        ...
        "blockers":["canonical_rejects_leave_unresolved_cross_split_links_and_eligibility",
    ...
    json_output(output_root/"holdout.json",{
        "status":"provisional_engineering_source_partition_holdout_not_scientific_freeze","inputs":inputs,
    ...
    json_output(output_root/"b4-observed-group-summary.json",{
        "status":"provisional_observed_groups_not_scientific_split_acceptance","inputs":inputs,
...
if __name__=="__main__":
    ...
    run(args.manifest,args.dev,args.metadata,args.canonical,args.canonical_summary,args.config,args.output_root)
