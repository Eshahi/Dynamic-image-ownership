...
Three focused model-free tests cover literal revision-bound rank vectors, permutation/all-member preservation, malformed groups/UUIDs and duplicate source identity. Use the verified workflow interpreter with `scripts/b3_source_candidate_order.py --eligibility '<existing ignored private index>' --output '<new ignored pool path>'`. Full workflow suite: **175 tests, 167 pass/eight expected dependency skips**; `git diff --check` passed. Actual independent actor `/root/b3_intake_review` reviewed exact `8f9dafd967ee2f095c0c646e1b082219eb9cb834`, independently recomputed every rank/order/member, reversed actual input mappings and fully replayed ZIP1467/558 integrity with no narrow finding. See [review record](../audits/b3-source-order-review-20260926/review.md). Final source/study/content/rights/science acceptance flags remain false; the official run stays paused at plan-acceptance and B3 stays open.
