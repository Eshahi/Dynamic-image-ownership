...
Status 2026-09-24: **documented source conflict, no dataset selection or acquisition**. Issue #10 (B3) depends on A6/#7, which remains open. This early read-only preflight does not start the B3 manifest, change the proposal, choose sample IDs, clear image rights, or authorize scientific compute. Apply the [research contract](research-contract.md), [scope guard](scope-guard.md), and [approval policy](approval-policy.md).
...
The user must select or authorize a material change to the final dataset commitment under `research/approval-policy.md` and `research/scope-guard.md`. Until then, do not freeze `data/manifest.csv`, `data/dev-ids.json`, or `data/datasheet.md` with a silently substituted COCO release. Independent A6 and non-COCO preparatory work may continue. Dataset acquisition, licenses per image, IDs/splits and hash receipts remain separate B3/B4 work after the dependency and decision are resolved.
