...
Hashes identify local observer notes, not remote paper authenticity or scientific truth. Targeted source descriptions are not reproduced results. See README.md for comparison definitions and final dispositions.
...
- B1-SEMBIND: SemBind combines learned semantic masking with latent watermarking and inversion. [sembind-v1; supporting; direct-evidence; confidence high]. Limitations: Source-specific mechanism context only.

- B1-SLICE: SLICE verifies factorized semantic binding through diffusion inversion. [slice-v1; supporting; direct-evidence; confidence high]. Limitations: Source-specific mechanism context only.

- B1-IMAGE-DCT: This image-DCT method does not establish the proposed latent-to-image bridge. [paunwala2012; supporting; agent-inference; confidence high]. Limitations: Source-specific mechanism context only.
