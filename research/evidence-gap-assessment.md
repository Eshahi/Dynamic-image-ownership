...
Issue #8. 2026-09-23. This maps all 42 immutable `claims.csv` IDs to the admitted `literature/method-checkpoint/evidence-ledger.jsonl`. It is an assessment layer, not a replacement claim ledger or experiment result. Source statements and their locators/limitations remain in that ledger and its paper cards.
...
Every relationship is an author inference about relevance, not direct support of the original thesis proposition. In particular, comparator existence and threat motivation do not validate the proposed mechanism. Empty evidence IDs mean no admitted mapping in this seven-card package, not proof of a global literature gap. The response's separate B1 claim IDs are retained and referenced explicitly. Requirements need provenance and implementation/design work; they need not all be proved by literature.
...
5. RQ-02/HYP-02: isolated pixel/latent papers do not establish a causal survival advantage. The mapped decoder entry identifies a candidate, not its embedding domain; admit explicit encoder-domain evidence before selecting a pixel baseline. A matched regeneration study and a primary regeneration source remain needed.
...
Inspect regeneration and qualifying/contrary evidence, version differences and SEAL code; resolve metadata-only source access or explicitly bound its use. Consolidate required B1 outputs once coverage is adequate and obtain an independent whole-package review. Do not accumulate more snapshots for unchanged sources. This mapping's completeness is documentary coverage only, not readiness for the evidence gate or permission for compute.
